...
class TomlLoader {
    ...
    async loadTomlParser() {
        ...
        if (window.toml && window.toml.parse) {
            // Using existing window.toml parser
            return window.toml;
        }
        ...
        // No TOML parser found immediately, waiting for scripts to load
        await new Promise(resolve => setTimeout(resolve, 500));

        // Check again after waiting
        if (window.toml && window.toml.parse) {
            // Using window.toml parser after wait
            return window.toml;
        }
        ...
        // No existing TOML parser found, trying simple fallback parser instead of CDNs

        // Instead of trying CDNs (which are failing), go directly to simple parser
        // Creating simple inline TOML parser fallback
    ...
    createSimpleTomlParser() {
        ...
        return {
            ...
            parse: (tomlString) => {
                // Using simple TOML parser fallback
